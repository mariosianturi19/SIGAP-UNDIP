...
export default function MyShiftsCalendar() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Jadwal Shift Saya</h1>
          <p className="text-gray-500 mt-1">Lihat jadwal shift Anda untuk minggu ini dan yang akan datang</p>
          {shiftsData && (
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent className="p-6">
              ...
              {currentWeekOffset === 0 && (
                <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-medium text-blue-900">Status Hari Ini</h3>
                    ...
                    >
                      <div className="flex flex-col items-center text-center">
                        <div className="w-full mb-3">
                          <h3 className={`font-semibold ${scheduleItem.is_today ? 'text-blue-900' : 'text-gray-900'}`}>
                            {scheduleItem.day_name}
                          </h3>
                          <p className={`text-sm ${scheduleItem.is_today ? 'text-blue-700' : 'text-gray-500'}`}>
                            {scheduleItem.date_formatted}
                          </p>
                        </div>
                        
                        <Badge className={`${status.color} flex items-center px-2 py-1 text-xs border mb-2`}>
                          {status.icon}
                          <span>{status.text}</span>
                        </Badge>
                        
                        {scheduleItem.is_scheduled && (
                          <div className="text-xs text-gray-500 mt-1">
                            ID Shift: {scheduleItem.shift_id}
                          </div>
                        )}
                      </div>
                    </motion.div>
              ...
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                <h3 className="font-medium text-gray-900 mb-3">Ringkasan Jadwal</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Total Hari Bertugas:</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">Pola Mingguan:</span>
              ...
              {shiftsData.upcoming_shifts.length > 0 && (
                <div className="mt-6">
                  <h3 className="font-medium text-gray-900 mb-3">Shift Mendatang</h3>
                    ...
                    {shiftsData.upcoming_shifts.map((shift) => (
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span className="font-medium text-blue-900">{shift.day_name}</span>
                          <span className="text-blue-700 ml-2">{shift.date_formatted}</span>
                        </div>

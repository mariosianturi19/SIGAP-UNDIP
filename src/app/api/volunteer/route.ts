...
export async function GET(request: NextRequest) {
  try {
    ...
    log("Fetching volunteers with auth:", authHeader.substring(0, 20) + "...");

    // Try multiple endpoints for volunteers
    const endpoints = [
      buildApiUrl("/admin/relawan"),
      buildApiUrl("/admin/users?role=volunteer"),
      buildApiUrl("/relawan")
    ];
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        log(`Response text from ${endpoint}:`, responseText.substring(0, 500));
        ...
        if (response.ok) {
          ...
          try {
            ...
            log(`Successfully parsed data from ${endpoint}:`, Object.keys(data));
            return NextResponse.json(data, { status: response.status });
          } catch (e) {
            logError(`Failed to parse JSON from ${endpoint}:`, e);
            continue;
          }
        } else {
          log(`Endpoint ${endpoint} failed with status ${response.status}`);
          lastError = responseText;
        }
      } catch (error) {
        log(`Error with endpoint ${endpoint}:`, error);
        lastError = error;
      }
    }
    ...
    logError("Volunteer fetch error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    log("Creating volunteer with data:", { ...body, password: "[HIDDEN]" });
    ...
    const response = await fetch(buildApiUrl("/admin/register-relawan"), {
      method: "POST",
      headers: {
        "Authorization": authHeader,
        "Content-Type": "application/json",
    ...
    log("Create volunteer response status:", response.status);

    const responseText = await response.text();
    log("Create volunteer response text:", responseText);

    // Check if response is HTML (error page)
    if (responseText.includes('<!DOCTYPE html>') || responseText.includes('<html>')) {
      logError("Received HTML instead of JSON when creating volunteer");
      return NextResponse.json(
    ...
    try {
      ...
      logError("Failed to parse create volunteer response as JSON:", e);
      return NextResponse.json(
    ...
    logError("Create volunteer error:", error);
    const errorMessage = (error instanceof Error) ? error.message : String(error);
    return NextResponse.json(

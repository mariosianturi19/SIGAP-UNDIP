...
export async function GET(request: NextRequest) {
  try {
    ...
    log("Fetching relawan panic reports from:", apiUrl);
    ...
    log("Get relawan panic reports response:", responseText);

    let data;
    try {
      data = JSON.parse(responseText);
    } catch (e) {
      logError("Failed to parse relawan panic reports response as JSON:", e);
      return NextResponse.json(
    ...
    logError("Get relawan panic reports error:", error);
    return NextResponse.json(
